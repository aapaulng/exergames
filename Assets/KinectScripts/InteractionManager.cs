...
//	public float SensorHeight = 1.0f;
    ...
    public Text UIdebugText;  //extra paul
...
//		handCursor = GameObject.Find("HandCursor");
	...
	void StartInteraction() 
	{
		...
		catch(DllNotFoundException ex)
		{
			...
			if(UIdebugText != null)
				UIdebugText.text = "Please check the Kinect SDK installation.";
		}
		catch (Exception ex) 
		{
		...
		if(UIdebugText)
		{
			...
			if(IsRightHandPrimary())
			{
				...
				else if(lastRightHandEvent == InteractionWrapper.InteractionHandEventType.Release)
				{
                    ...
                    //ExecuteEvents.Execute(StartGame.gameObject, pointer, ExecuteEvents.pointerEnterHandler);
                    //ExecuteEvents.Execute(StartGame.gameObject, pointer, ExecuteEvents.pointerUpHandler);
					//bPullRightHandFinished = false;
				}
				...
				if(isRightHandClick)
				{
					sGuiText += "  RightClick";
                    //ExecuteEvents.Execute(StartGame.gameObject, pointer, ExecuteEvents.pointerEnterHandler);
                    //ExecuteEvents.Execute(StartGame.gameObject, pointer, ExecuteEvents.pointerDownHandler);
                    ExecuteEvents.Execute(StartGame.gameObject, pointer, ExecuteEvents.submitHandler);
				}
...
//				}
		...
		if(handCursor != null)
		{
            ...
            //if(handCursor && handCursor.raw && texture)
            //{
            ...
            {
                //handCursor.guiTexture.texture = texture;
                //handCursor.GetComponent<RawImage>().mainTexture = texture;
                handCursor.transform.position = new Vector3(cursorScreenPos.x*Screen.width, cursorScreenPos.y*Screen.height, 0f);
                Debug.Log("x=" + cursorScreenPos.x + "y=" + cursorScreenPos.y);
                //handCursor.transform.position = new Vector3(300, 300, 0);
            }
		}
	}
