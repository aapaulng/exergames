...
{
    //Extra 
    public GameObject GameControllerCountDown;
...
//				break;
	...
	{
        //Extra Paul Here
        if(GameControllerCountDown)
        GameControllerCountDown.GetComponent<CountDownScript>().enabled = false;
		...
		if(ComputeUserMap)
		{
			...
			usersPrevState = new ushort[usersMapSize];  //Paul Come Here Change the UserMap Display
...
//				usersMapRect.x -= cameraRect.width * DisplayMapsWidthPercent; //usersClrTex.width / 2;
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor1 != null)
								{
                                    ...
                                    //HandCursor1.transform.position = new Vector3(100, 100);
								}
    ...
    void CalibrateUser(uint UserId, int UserIndex, ref KinectWrapper.NuiSkeletonData skeletonData)
    {
		...
		if(AllPlayersCalibrated)
		{
            ...
            //Paul Here
            //if(GameControllerCountDown)
            //GameControllerCountDown.GetComponent<CountDownScript>().enabled = true;
